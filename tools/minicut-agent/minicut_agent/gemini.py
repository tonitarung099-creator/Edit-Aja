...
class GeminiClient:
    ...
    def test_connection(self) -> dict[str, Any]:
        ...
        try:
            ...
            parsed = {"raw": text}
        return {"ok": True, "model": self.model, "response": parsed, "usage": self.usage.__dict__.copy()}
    ...
    def verify_candidates(
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            for label, offset in (("sebelum", -4000), ("dekat-sebelum", -450), ("sesudah", 4000)):
                jpg = extract_frame_jpeg(
                    ffmpeg, source, max(0, candidate.time_ms + offset), frame_width
                )
                parts.append({"text": f"Kandidat {i} · frame {label}"})
                parts.append({
                    "inline_data": {
        ...
        result["selected_time_ms"] = selected.time_ms
        result["selected_time"] = format_ms(selected.time_ms)
        result["target_ms"] = target_ms
        result["target"] = format_ms(target_ms)
        result["usage"] = self.usage.__dict__.copy()
        return result

def extract_frame_jpeg(ffmpeg: str, source: Path, time_ms: int, width: int = 512) -> bytes:
    cmd = [
...
def _response_text(data: dict[str, Any]) -> str:
    ...
    if not candidates:
        ...
        raise RuntimeError("Gemini tidak menghasilkan kandidat respons: " + json.dumps(feedback, ensure_ascii=False))
...
def _verification_prompt(
    ...
    return f"""
Tugas: pilih SATU batas part film paling natural dari kandidat yang sudah ditemukan MiniCut lokal.

Patokan global: {format_ms(target_ms)}.
Patokan sekitar 15 menit hanyalah referensi, BUKAN waktu potong wajib.
Prioritas tertinggi adalah keutuhan scene dan alur, bukan kedekatan ke patokan.

Aturan:
- Jangan memilih pergantian shot/kamera biasa jika percakapan atau kejadian masih satu scene.
- Jangan memotong di tengah dialog, subtitle aktif, aksi penting, reaksi, sebab-akibat, atau transisi yang masih satu rangkaian.
- Visual adalah sumber utama. SRT hanya alat verifikasi dialog.
- Frame yang diberikan adalah konteks ringkas di sekitar kandidat, bukan seluruh film.
- Jika tidak ada kandidat yang benar-benar aman, pilih kandidat paling aman tetapi set confidence rendah dan needs_review=true.
- Jangan mengarang timestamp lain; hanya boleh memilih salah satu kandidat yang tersedia.
